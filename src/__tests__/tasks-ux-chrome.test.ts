...
describe("resolveGuardedRoute", () => {
  it("redirects hidden routes and desktop-only routes on a phone, before render", () => {
    ...
    // Chat ships on mobile (boards pass), so a room is never bounced for being a phone.
    expect(resolveGuardedRoute("/app/discussions/abc", {}, true)).toBeNull();
  ...
  it("lets allowed routes render and never loops onto itself", () => {
    ...
    // With Inbox and Calendar hidden, Chat is the remaining landing-capable route on a phone.
    expect(resolveGuardedRoute("/app/inbox", { hidden_nav_items: ["/app/inbox", "/app/calendar"] }, true)).toBe("/app/discussions");
    expect(resolveGuardedRoute("/app/inbox", { hidden_nav_items: ["/app/inbox", "/app/calendar", "/app/discussions"] }, true)).toBeNull();
...
describe("ThemeToggle", () => {
  ...
  it("uses tokens and dark variants, not zinc/gray hex classes", () => {
    ...
    // The settings pass rebuilt the toggle as a radiogroup with a bg-muted thumb.
    expect(src).toContain("rounded-lg bg-muted");
