...
const PHOTO_COVER_CATEGORIES: { label: string; photos: PhotoCover[] }[] = [
  {
    label: "Cal Berkeley",
    photos: [
...
const PHOTO_COVERS: PhotoCover[] = PHOTO_COVER_CATEGORIES.flatMap((cat) => cat.photos);
          ...
          <div className="relative bg-popover rounded-2xl shadow-2xl border border-border w-full max-w-md mx-4 animate-announce-card-in overflow-hidden">
            ...
            <div className="p-4 space-y-4 max-h-[50vh] overflow-y-auto">
              ...
              {PHOTO_COVER_CATEGORIES.map((cat) => (
                <div key={cat.label}>
                  <p className="text-xs font-medium text-foreground mb-2">{cat.label}</p>
              ...
              {/* Gradients */}
              <div>
                <p className="text-xs font-medium text-foreground mb-2">Gradients</p>
              ...
              {/* Solid Colors */}
              <div>
                <p className="text-xs font-medium text-foreground mb-2">Solid Colors</p>
                <div className="grid grid-cols-5 gap-2">
                  {SOLID_COVERS.map((p) => (
                    <button
                      key={p.id}
