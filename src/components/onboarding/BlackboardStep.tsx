...
/**
 * Loose client-side check: an HTTPS URL. Blackboard Learn feed URLs are
...
const BLACKBOARD_STEPS: Array<{ text: string }> = [
  { text: "Open Blackboard and go to Calendar" },
  { text: "Click \"Get External Calendar Link\" at the bottom left" },
...
/**
 * Blackboard Learn onboarding step. Theme-aware, mirrors PensieveStep but
 * URL-only: Blackboard has no per-course selection in the sync backend, so
 ...
 */
export default function BlackboardStep({ onNext, onSkip, saving, error, setError, initialUrl, onDraftChange, skipLabel = "Skip for now" }: BlackboardStepProps) {
  ...
  async function handleSaveAndNext() {
    ...
    if (!trimmed) {
      setUrlInvalid(true);
      showToast("Please enter your Blackboard calendar URL.", { variant: "error", duration: 4000 });
      return;
    }
    ...
    setUrlInvalid(false);
    setError(null);
    await onNext({ blackboard_calendar_url: trimmed });
  }

  return (
    <div className="text-center">
      <div className="flex items-center justify-center gap-2 mb-4">
        <img src="/blackboard-logo.svg" alt="" className="w-6 h-6 object-contain animate-drop-in" />
        <h2 className="text-lg font-bold text-foreground animate-drop-in">Blackboard</h2>
      </div>

      {/* Numbered instruction steps */}
      <div className="animate-drop-in delay-100">
        <div className="flex flex-col gap-1 mb-4 text-left">
          {BLACKBOARD_STEPS.map((step, i) => (
