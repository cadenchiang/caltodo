...
interface BrightspaceSettingsProps {
...
/**
 * Brightspace (D2L) integration row card. Mirrors PensieveSettings: logo tile +
 * title + description + connect/disconnect. Brightspace connects via an iCal
 * feed URL entered in the standalone ?setup=brightspace flow.
 *
 ...
 */
export default function BrightspaceSettings({ credentials, onUpdate }: BrightspaceSettingsProps) {
  const router = useRouter();
  const { showToast } = useToast();
  const { tasks, deleteTasksBySource } = useTaskContext();
  const [disconnecting, setDisconnecting] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  const isConnected = Boolean(credentials.brightspace_calendar_url);
  const sourceTaskCount = tasks.filter((t) => t.source === "brightspace").length;

  /** Disconnects Brightspace by clearing the calendar URL via API. */
  async function handleDisconnect() {
    ...
    try {
      const res = await fetch("/api/credentials", {
        ...
        body: JSON.stringify({ brightspace_calendar_url: null }),
      });
      if (!res.ok) {
      ...
      onUpdate(updated);
      await deleteTasksBySource("brightspace");
      showToast("Brightspace disconnected.");
    } catch (err) {
      ...
      <div className="flex items-center gap-2.5 sm:gap-3.5">
        ...
        {isConnected ? (
          <button
            ...
            aria-label="Disconnect Brightspace"
          ...
          <button
            onClick={() => router.push("/app/onboarding?setup=brightspace")}
          ...
          <div className="relative bg-card rounded-2xl border border-border shadow-2xl w-[calc(100%-2rem)] max-w-sm p-6 animate-modal-in">
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                {sourceTaskCount > 0
                  ? `This will remove ${sourceTaskCount === 1 ? "1 synced task" : `${sourceTaskCount} synced tasks`} from Brightspace. You can reconnect later to sync them again.`
